...
<img src="./screenshots/hero.png" width="800" alt="LabLens Dashboard"/>
...
| **Protocol Card** | Reconstructed experiment summary organized as: Materials -> Methods -> Conditions -> Instruments -> Parameters. |
...
| **Zero Cloud Dependencies** | Runs entirely locally using spaCy `en_core_web_sm`. No API keys, no external services, no data leaves your machine. |
...
<table>
<tr>
<td><img src="./screenshots/hero.png" width="400" alt="Dashboard overview"/></td>
<td><img src="./screenshots/feature1.png" width="400" alt="Entity table with filters"/></td>
</tr>
<tr>
<td><strong>Dashboard Overview</strong> -- Metric cards, entity breakdown by category, confidence-coded entity cards</td>
...
<tr>
<td><img src="./screenshots/feature2.png" width="400" alt="Interactive review panel"/></td>
<td><img src="./screenshots/feature3.png" width="400" alt="Export panel"/></td>
</tr>
<tr>
<td><strong>Interactive Review</strong> -- Accept or reject entities one by one with paginated navigation</td>
<td><strong>Export Panel</strong> -- Download in JSON, CSV, or BioSchemas JSON-LD with inline previews</td>
</tr>
</table>
</div>
...
graph TD
    ...
    subgraph Scoring["Scoring Engine"]
        ...
        SIGMOID[Sigmoid Squash<br/>range: 0.1 to 0.99]
...
|----------|-------------|------------------|:---:|
| **Instrument** | Microscopes, spectrometers, analytical equipment | SEM, TEM, XRD, FTIR, NMR, AFM | 40+ |
| **Chemical** | Reagents, solvents, compounds, buffers | H2O, NaCl, ethanol, PBS, DMSO, TiO2 | 50+ |
| **Condition** | Experimental conditions and environmental parameters | 25 C, pH 7.4, 1 atm, 8000 rpm, 30 min | 30+ |
| **Parameter** | Measurement values, instrument settings | 532 nm, 200 kV, 10 kHz, 15 nm diameter | 25+ |
| **Material** | Substrates, scaffolds, composites, raw materials | graphene, PLGA, collagen, silicon wafer | 40+ |
| **Method** | Synthesis techniques, analytical methods | CVD, spin coating, hydrothermal, sol-gel | 35+ |
| **Organism** | Biological species, cell lines, microbial strains | E. coli, HeLa, mesenchymal stem cells | 20+ |
| **Sample** | Physical forms, preparations, specimen types | thin film, nanoparticles, scaffold, powder | 15+ |
...
The app loads with **Demo Samples** selected by default -- click through to see extraction results immediately.
...
The Docker image uses a multi-stage build with a non-root `appuser` and a built-in healthcheck on `/_stcore/health`.
...
```
======================================================================
Document: TEM Study: Fe3O4 Nanoparticles
Text length: 3,247 chars | Extraction time: 142 ms | Total entities: 64
======================================================================
...
bioschemas = result.to_bioschemas()
```
...
Every entity receives a composite confidence score in **[0.1, 0.99]**, computed from four independent signals:

| Signal | Weight | What It Measures |
|--------|:------:|------------------|
| **Specificity** | 0.30 | Longer, more specific entity text scores higher |
| **Context** | 0.25 | Presence of category-specific action verbs near the entity |
| **Frequency** | 0.25 | Entities mentioned 3+ times in the document score higher |
| **Source Agreement** | 0.20 | Entities found by both spaCy NER and regex receive the highest score |
...
Full extraction results with metadata, entity details, and document-level statistics.
...
Flat tabular format with one row per entity. Columns: `document`, `text`, `category`, `subcategory`, `confidence`, `context`, `verified`.

### BioSchemas JSON-LD

[Schema.org](https://schema.org)-compatible linked data for FAIR interoperability:
...
|------|---------|---------|
| **Python** | 3.12+ | Runtime |
| **spaCy** | 3.7+ | Named Entity Recognition with `en_core_web_sm` model |
| **Streamlit** | 1.30+ | Web UI framework with custom CSS and interactive review |
| **pdfplumber** | 0.11+ | PDF text extraction with layout-aware parsing |
| **pandas** | 2.1+ | DataFrame operations for CSV export and entity tables |
| **regex** | 2024.1+ | Unicode-aware scientific pattern matching |
...
│   ├── sample_papers.py     # Built-in synthetic demo papers
